...
template <typename InputIterator, typename T>
auto count( InputIterator beg, InputIterator end,
            const T &value ) -> decltype( beg - beg )
{
    auto num = beg - beg;
    while( beg != end ) {
        if( *beg++ == value ) {
...
template <typename InputIterator, typename UnaryPredicate>
auto count_if( InputIterator beg, InputIterator end,
               UnaryPredicate predicate ) -> decltype( beg - beg )
{
    auto num = beg - beg;
    while( beg != end ) {
        if( predicate( *beg++ ) ) {
...
bool is_permutation( ForwardIterator1 beg1, ForwardIterator1 end1, ForwardIterator2 beg2, 
    ...
    for( auto scan = beg1; scan != end1; ++scan ) {
        ...
        if( matches == 0 || 
            matches != mystl::count_if( scan, end1, std::bind( predicate, _1, *scan ) ) ) {
            return false;            
        }
...
OutputIterator remove_copy( InputIterator srcBeg, InputIterator srcEnd, 
                            OutputIterator desBeg, const T &elem ) {
    while( srcBeg != srcEnd ) {
        auto copy = *srcBeg++;
        if( copy != elem ) {
            *desBeg++ = copy;
...
OutputIterator remove_copy_if( InputIterator srcBeg, InputIterator srcEnd, 
                               OutputIterator desBeg, UnaryPredicate predicate ) {
    while( srcBeg != srcEnd ) {
        auto copy = *srcBeg++;
        if( !predicate( copy ) ) {
            *desBeg++ = copy;
...
OutputIterator unique_copy( InputIterator srcBeg, InputIterator srcEnd, 
                            OutputIterator desBeg, BinaryPredicate predicate ) {
    if( srcBeg == srcEnd ) {
        return desBeg;
    }
...
OutputIterator set_union( InputIterator1 srcBeg1, InputIterator1 srcEnd1,
                          InputIterator2 srcBeg2, InputIterator2 srcEnd2,
...
template <typename InputIterator1, typename InputIterator2, 
          typename OutputIterator, typename Comp>
OutputIterator set_difference( InputIterator1 srcBeg1, InputIterator1 srcEnd1, 
                               InputIterator2 srcBeg2, InputIterator2 srcEnd2, 
                               OutputIterator desBeg, Comp comp ) {
    while( srcBeg1 != srcEnd1 && srcBeg2 != srcEnd2 ) {
        if( comp( *srcBeg1, *srcBeg2 ) ) {
...
template <typename InputIterator1, typename InputIterator2, 
          typename OutputIterator, typename Comp>
OutputIterator set_intersection( InputIterator1 srcBeg1, InputIterator1 srcEnd1, 
                                 InputIterator2 srcBeg2, InputIterator2 srcEnd2,
                                 OutputIterator desBeg, Comp comp ) {
    while( srcBeg1 != srcEnd1 && srcBeg2 != srcEnd2 ) {
        if( comp( *srcBeg1, *srcBeg2 ) ) {
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator>
OutputIterator set_symmetric_difference( InputIterator1 srcBeg1, InputIterator1 srcEnd1,
                                         InputIterator2 srcBeg2, InputIterator2 srcEnd2, 
                                         OutputIterator desBeg ) {
    while( srcBeg1 != srcEnd1 && srcBeg2 != srcEnd2 ) {
        if( *srcBeg1 < *srcBeg2 ) {
...
OutputIterator reverse_copy( BidirectionalIterator beg, BidirectionalIterator end, 
                             OutputIterator desBeg ) {
    while( end != beg ) {
        *desBeg++ = *--end;
    }
...
OutputIterator rotate_copy( ForwardIterator beg, ForwardIterator newBeg, 
                            ForwardIterator end, OutputIterator desBeg ) {
    if( beg == end ) {
        return desBeg;
    }
    auto current = newBeg;
...
void shuffle( RandomIterator beg, RandomIterator end, 
